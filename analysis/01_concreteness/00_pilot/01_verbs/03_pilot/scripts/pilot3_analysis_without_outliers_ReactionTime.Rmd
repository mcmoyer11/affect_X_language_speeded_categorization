---
title: "Pilot 2: Corrected Analysis without outliers: ReactionTime Column"
author: "morgan moyer"
date: "`r Sys.Date()`"
...
center = df_factors %>%
  mutate(cTask = as.numeric(Task)-mean(as.numeric(Task)),
         cCorrectedAccuracy = as.numeric(CorrectedAccuracy)-mean(as.numeric(CorrectedAccuracy)),
...
# Does CorrectedAccuracy predict reaction time? 

In other words, is reaction time affected by certainty about the categorization?
- No.
```{r}

m = lmer(LogReactionTime ~ cCorrectedAccuracy + (1|ID.true) + (1|Word), data=center)
...
## On CorrectedAccuracy
- No.
```{r}
m = lmer(cCorrectedAccuracy ~ cBlockOrder + (1|ID.true) + (1+cBlockOrder|Word), data=center)
