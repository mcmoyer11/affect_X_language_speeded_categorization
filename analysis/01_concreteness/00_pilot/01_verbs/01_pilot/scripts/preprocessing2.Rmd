---
title: 'affect proj pilot1: preprocessing'
author: "morgan moyer"
date: "`r Sys.Date()`"
...
d <- d %>% 
    ...
    select(ID.true,Word,Label,ConcValCombo,Task,Group,Response,Accuracy,EventTime,Value,RT,ReactionTime,Key_value_F,Key_value_J,Comments) %>% 
  distinct()
...
unique(d.train$Key_value_F)

d.train <- d.train %>% 
    filter(Parameter == "Selection") %>%
    select(ID.true,Word,Label,Task,EventTime,Value,ReactionTime, Key_value_F,Key_value_J,Comments,Group) %>% 
  distinct()
