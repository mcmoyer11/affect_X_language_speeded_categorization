---
title: 'Pilot 2 Nouns: preprocessing'
author: "morgan moyer"
date: "`r Sys.Date()`"
...
# Read in the results
vc.a <- read.pcibex("../data/nouns2_conc-val_VC_A.csv")
vc.a["BlockOrder"] <- "VC"
vc.a["Group"] <- "A"
length(unique(vc.a$MD5.hash.of.participant.s.IP.address))

vc.b <- read.pcibex("../data/nouns2_conc-val_VC_B.csv")
vc.b["BlockOrder"] <- "VC"
vc.b["Group"] <- "B"
length(unique(vc.b$MD5.hash.of.participant.s.IP.address))

cv.a <- read.pcibex("../data/nouns2_conc-val_CV_A.csv")
cv.a["BlockOrder"] <- "CV"
cv.a["Group"] <- "A"
length(unique(cv.a$MD5.hash.of.participant.s.IP.address))

cv.b <- read.pcibex("../data/nouns2_conc-val_CV_B.csv")
cv.b["BlockOrder"] <- "CV"
cv.b["Group"] <- "B"
