...
ggsave("../graphs/exp1_mostacc_verbs_acc.pdf",width = 6, height = 3)
```
...
ggsave("../graphs/exp1_mostacc_verbs_rt.pdf",width = 6, height = 3)
```
...
agr <- d %>% 
  ...
  filter(Word %in% concrete_accuracy_tot$Word) %>% 
  filter(Word != "obsess") %>% 
  group_by(Word,Task) %>%
  summarize(MeanAccuracy = mean(Accuracy),
            MeanReactionTime = mean(ReactionTime))
...
ggsave("../graphs/total_accXrt.pdf",width = 8, height = 4)
```
...
aggregate(LogReactionTime ~ Task, data = center, mean)
tapply(cTask, center$Task, mean)
