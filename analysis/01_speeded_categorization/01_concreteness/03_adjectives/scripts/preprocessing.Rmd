---
title: 'Adjs Conc-Abs: preprocessing'
author: "morgan moyer"
date: "`r Sys.Date()`"
...
# Read in the results
vc.a <- read.pcibex("../data/AFH_adjs_conc-val_VC_A.csv")
vc.a["BlockOrder"] <- "VC"
length(unique(vc.a$MD5.hash.of.participant.s.IP.address))

vc.b <- read.pcibex("../data/AFH_adjs_conc-val_VC_B.csv")
vc.b["BlockOrder"] <- "VC"
length(unique(vc.b$MD5.hash.of.participant.s.IP.address))

cv.a <- read.pcibex("../data/AFH_adjs_conc-val_CV_A.csv")
cv.a["BlockOrder"] <- "CV"
length(unique(cv.a$MD5.hash.of.participant.s.IP.address))

cv.b <- read.pcibex("../data/AFH_adjs_conc-val_CV_B.csv")
cv.b["BlockOrder"] <- "CV"
length(unique(cv.b$MD5.hash.of.participant.s.IP.address))

facts <- read.csv("../data/adjectives_weighted.csv")
