---
title: 'Animacy Nouns: preprocessing'
author: "morgan moyer"
date: "`r Sys.Date()`"
...
# Read in the results
vc.a <- read.pcibex("../data/AFH_nouns_animacy_AV_A.csv")
vc.a["BlockOrder"] <- "AV"
# vc.a["Group"] <- "A"
length(unique(vc.a$MD5.hash.of.participant.s.IP.address))

vc.b <- read.pcibex("../data/AFH_nouns_animacy_AV_B.csv")
vc.b["BlockOrder"] <- "AV"
# vc.b["Group"] <- "B"
length(unique(vc.b$MD5.hash.of.participant.s.IP.address))

cv.a <- read.pcibex("../data/AFH_nouns_animacy_VA_A.csv")
cv.a["BlockOrder"] <- "VA"
# cv.a["Group"] <- "A"
length(unique(cv.a$MD5.hash.of.participant.s.IP.address))

cv.b <- read.pcibex("../data/AFH_nouns_animacy_VA_B.csv")
cv.b["BlockOrder"] <- "VA"
# cv.b["Group"] <- "B"
length(unique(cv.b$MD5.hash.of.participant.s.IP.address))

facts <- read.csv("../data/nouns_ani_tot.csv")
...
d.train <- d %>% 
  filter(Label %in% c("calibrator_val_feedback","calibrator_ani_feedback","calibrator_val_no_feedback","calibrator_ani_no_feedback"))

d <- d %>%
  filter(Label %in% c("test_val","test_ani")) %>% 
...
d$Animacy <- as.character(d$Animacy)
d$Valence <- as.character(d$Valence)
table(d$Label)


# Add Task column
d$Task <- ifelse(d$Label == "test_val", "Valence", 
                 ifelse(d$Label == "test_ani", "Animacy", "Na"))
...
d <- d %>%
  ...
  mutate(
    Accuracy = case_when(
      Task == "Valence" ~ as.numeric(grepl(Response, as.character(Valence), fixed = TRUE)),
      Task == "Animacy" ~ as.numeric(grepl(Response, as.character(Animacy), fixed = TRUE)),
      TRUE ~ NA_real_
    )
  ) %>%
  ungroup()
...
table(d$Accuracy,d$Response)

d <- d %>% 
    filter(Label %in% c("test_ani","test_val")) %>% 
    filter(Parameter == "Selection") %>%
    select(ID.true,Word,Label,Animacy,Valence,Task,BlockOrder,Group,Response,Accuracy,EventTime,Value,RT,ReactionTime,Key_value_F,Key_value_J,Comments) %>% 
  distinct()
...
d.train$Task <- ifelse(d.train$Label %in% c("calibrator_val_feedback","calibrator_val_no_feedback"), "Valence", 
                 ifelse(d.train$Label %in% c("calibrator_ani_feedback","calibrator_ani_no_feedback"), "Animacy", "Na"))
