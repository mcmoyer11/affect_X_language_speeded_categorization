---
title: "Adjs Soc-Phys Weighted/Normed: Reaction Time Graphs"
author: "morgan moyer"
date: "`r Sys.Date()`"
output: pdf_document
---
...
agr <- df.outliers.removed %>% 
  ...
  summarize(MeanReactionTime = mean(ReactionTime),
            SD = sd(ReactionTime),
            MeanLogReactionTime = mean(LogReactionTime))
print(agr)
```
...
ggsave("../graphs/exp2b_paper.jpg",width = 6, height = 3)
...
ggsave("../graphs/exp2b_items.pdf",width = 8, height = 6)
```
...
ggplot(agr,aes(x=Task, y=MeanReactionTime, alpha=Task, fill=BlockOrder)) +
  geom_bar(position="dodge",stat="identity") +
  geom_errorbar(aes(ymin=YMin,ymax=YMax),width=.25,position=position_dodge(0.9)) +
