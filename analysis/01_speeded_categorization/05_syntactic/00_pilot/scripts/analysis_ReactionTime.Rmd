---
title: "Pilot Syntactic: Analysis without outliers: ReactionTime "
author: "morgan moyer"
date: "`r Sys.Date()`"
...
center = df_factors %>%
  mutate(cTask = as.numeric(Task)-mean(as.numeric(Task)),
         cAccuracy = as.numeric(Accuracy)-mean(as.numeric(Accuracy)),
         cCategory = as.numeric(Category)-mean(as.numeric(Category)),
...
# Is there an Interaction between Task and Syntactic Category?
No.
```{r}

m = lmer(LogReactionTime ~ cTask*cCategory + (1+cCategory+cTask|ID.true) + (1+cTask|Word), data=center)
...
# Three-way interaction?Y
Yes.
```{r}

# m = lmer(LogReactionTime ~ cTask*cCategory*ConcValCombo + (1+cCategory+ConcValCombo+cTask|ID.true) + (1+cTask|Word), data=center)
# saveRDS(m, "../models/model-Task-ConcValCombo-Category.rds")

m <- readRDS("../models/model-Task-ConcValComb-Category.rds")
summary(m)
```
...
sem <- df_factors %>% 
  filter(Task == "Syntactic") %>% 
  mutate(
         Syntactic = ifelse(grepl("verb", Category), "verb", 
                    ifelse(grepl("noun", Category), "noun", NA)),
...
```{r}
m = lmer(LogReactionTime ~ cCategory + (1+cCategory|ID.true) + (1|Word), data=sem)
summary(m)
