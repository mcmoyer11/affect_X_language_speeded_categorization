---
title: "Adjs Subj-Obj: Graphs for Accuracy"
author: "morgan moyer"
date: "`r Sys.Date()`"
...
conc <- d %>% 
  ...
  # filter(Word %in% conc$Word) %>% 
  group_by(Word,ConcValCombo) %>%
  mutate(Response.n = as.numeric(factor(Response, levels = c("subjective", "objective"))) - 1) %>%  # Convert to numeric and subtract 1
  summarize(PropObjective = mean(Response.n))
  # filter(PropPositive > .1 | PropPositive < .9)

dodge = position_dodge(.9)
ggplot(data=conc, aes(x=reorder(Word,PropObjective),y=PropObjective,fill=ConcValCombo)) +
  geom_bar(position=dodge,stat="identity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
  # guides(fill = "none")
```
