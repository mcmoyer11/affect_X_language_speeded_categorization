---
title: 'Categorization Task: Accuracy Exploration'
author: "author"
date: "`r Sys.Date()`"
...
```{r, run=FALSE, eval=FALSE, echo=FALSE}
cat <- d[,c("Word","ID.true","Experiment","Task","LogReactionTime","Response","Accuracy","ConcValCombo")]
cat$ExperimentType <- "Categorization"
write.csv(cat,"../data/categorization.csv")
```
...
ggplot(agr, aes(x = MeanReactionTime, y = MeanAccuracy, color = Task)) +
  ...
  ggtitle("Words Where Accurary on Concept \u2265 Accuracy on Valence for each Exp") +
  theme(
...
# ggsave("../graphs/total_accXrt_poster.pdf",width = 8, height = 4)
...
summary(m)
m.simple <- lmer(LogReactionTime ~ cTask*Experiment + (1+cTask|Word) + (1+cTask|ID.true), data = dcen)
summary(m.simple)

anova(m,m.simple)
```

## Nested model comparison: Does Accuracy 

Likelihood-ratio tests revealed that Task significantly improved model fit relative to a baseline model (χ² = 51.1, p < .001), whereas Accuracy alone did not. Adding Accuracy and its interactions to the Task model yielded a further substantial improvement in fit (χ² = 83.7, p < .001), indicating that Accuracy modulates rather than replaces Task effects on reaction time.
```{r}
m_base <- lmer(
  LogReactionTime ~ Experiment +
  (1 + cTask | Word) + (1 + cTask | ID.true),
  data = dcen, REML = FALSE
)

m_task <- lmer(
  LogReactionTime ~ cTask + Experiment +
  cTask:Experiment +
  (1 + cTask | Word) + (1 + cTask | ID.true),
  data = dcen, REML = FALSE
)
...
m_acc <- lmer(
  LogReactionTime ~ cAccuracy + Experiment +
  cAccuracy:Experiment +
  (1 + cTask | Word) + (1 + cTask | ID.true),
  data = dcen, REML = FALSE
)

m_both <- lmer(
  LogReactionTime ~ cAccuracy * cTask * Experiment +
  (1 + cTask | Word) + (1 + cTask | ID.true),
  data = dcen, REML = FALSE
)

anova(m_base, m_task, m_acc, m_both)
...
sub <- d %>% 
  filter(Word %in% concept_acc$Word) %>% 
  filter(Experiment %in% c("Ex2b: Phys-Soc (Adjs)","Ex4: Syntactic (N v. V)"))
...
conc <- sub %>%
  filter(Word %in% concept_acc$Word) %>% 
  filter(Experiment == "Ex2b: Phys-Soc (Adjs)") %>%
  filter(Task == "SocPhys") %>% 
...
conc <- sub %>%
  filter(Word %in% concept_acc$Word) %>% 
  filter(Experiment == "Ex4: Syntactic (N v. V)") %>%
