---
title: 'Adjs Subj-Ovj: preprocessing'
author: "morgan moyer"
date: "`r Sys.Date()`"
...
# Read in the results
vc.a <- read.pcibex("../data/AFH_adjs_subj-obj_VS_A.csv")
vc.a["BlockOrder"] <- "VS"
length(unique(vc.a$MD5.hash.of.participant.s.IP.address))

vc.b <- read.pcibex("../data/AFH_adjs_subj-obj_VS_B.csv")
vc.b["BlockOrder"] <- "VS"
length(unique(vc.b$MD5.hash.of.participant.s.IP.address))

cv.a <- read.pcibex("../data/AFH_adjs_subj-obj_SV_A.csv")
cv.a["BlockOrder"] <- "SV"
length(unique(cv.a$MD5.hash.of.participant.s.IP.address))

cv.b <- read.pcibex("../data/AFH_adjs_subj-obj_SV_B.csv")
cv.b["BlockOrder"] <- "SV"
length(unique(cv.b$MD5.hash.of.participant.s.IP.address))

facts <- read.csv("../data/words.csv")
...
d$Task <- ifelse(d$Label == "test_val", "Valence", 
                 ifelse(d$Label == "test_so", "SubjObj", "Na"))

table(d$Task,d$Label)

d <- d %>% 
    filter(Label %in% c("test_so","test_val")) %>% 
