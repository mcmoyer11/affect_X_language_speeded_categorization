---
title: 'Adjs Soc-Phys Weighted/Normed: preprocessing'
author: "morgan moyer"
date: "`r Sys.Date()`"
...
# Read in the results
vc.a <- read.pcibex("../data/adjs4_soc-phys_VS_A.csv")
vc.a["BlockOrder"] <- "VS"
length(unique(vc.a$MD5.hash.of.participant.s.IP.address))

vc.b <- read.pcibex("../data/adjs4_soc-phys_VS_B.csv")
vc.b["BlockOrder"] <- "VS"
length(unique(vc.b$MD5.hash.of.participant.s.IP.address))

cv.a <- read.pcibex("../data/adjs4_soc-phys_SV_A.csv")
cv.a["BlockOrder"] <- "SV"
length(unique(cv.a$MD5.hash.of.participant.s.IP.address))

cv.b <- read.pcibex("../data/adjs4_soc-phys_SV_B.csv")
cv.b["BlockOrder"] <- "SV"
length(unique(cv.b$MD5.hash.of.participant.s.IP.address))

facts <- read.csv("../data/adjs_weighted_list.csv")
...
d.train <- d %>% 
  filter(Label %in% c("calibrator_val_feedback","calibrator_so_feedback","calibrator_val_no_feedback","calibrator_so_no_feedback"))
...
# Create ConcValCombo Column
d$ConcValCombo <- paste(d$SocPhys,d$Valence,sep='-')
...
d$Task <- ifelse(d$Label == "test_val", "Valence", 
                 ifelse(d$Label == "test_sp", "SocPhys", "Na"))

table(d$Task,d$Label)

d <- d %>% 
    filter(Label %in% c("test_sp","test_val")) %>% 
