Template(
    GetTable("ani_cal_A.csv").setGroupColumn("Group"),
    row => newTrial("calibrator_ani_feedback",
        ...
        newCanvas('myCanvas', 700, 300) // Reduced canvas height for better positioning
        ...
        newText("feedback", "")
            ...
            .center()
            .print()
            .remove(), // Initially hidden until updated

        // Define the `selectedKey` variable to capture user response
        newVar("selectedKey").global(),

        // Capture key press and store the selected key
        newSelector("Selection")
            .add(
                newText("Option_F", "F: " + row.F_value).settings.css("font-size", "25"),
                newText("Option_J", "J: " + row.J_value).settings.css("font-size", "25")
            )
            .keys("F", "J")
            .log()
            .wait()
        ...
        // Feedback message to display for all rows
        getText("feedback")
            .text("This word is best described as <strong>" + row.Animacy + "</strong>.")
            .center()
            .print(),

        newTimer("feedbackWait", 2000).start().wait(), // Display feedback for 2 seconds
