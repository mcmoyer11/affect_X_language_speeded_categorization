Template(
    GetTable("syn_cal.csv").setGroupColumn("Group"),
    row => newTrial("calibrator_syn_feedback",
        ...
        getVar("selectedValue").test.is(row.Category)
            .success(
                getText("feedback").text("This word is a <strong>" + row.Category + "</strong>.")
            )
            .failure(
                getText("feedback").text(
                    row.Category === 'neutral'
                        ? "This word is not really one or the other."
                        : "This word is a <strong>" + row.Category + "</strong>."
                )
            ),
